...
// Expand config (which modules should request expanded objects)
const EXPAND_DEFAULT = APP_CONFIG.EXPAND_DEFAULT ?? true
const EXPAND_MODULES = APP_CONFIG.EXPAND_MODULES || []
...
export const getAll = async (moduleKey, params = {}) => {
  ...
  // Add expand param for configured modules
  if (EXPAND_MODULES.includes(moduleKey)) {
    queryParams.append('expand', String(EXPAND_DEFAULT))
  }

  const response = await api.get(`${module.endpoint}?${queryParams.toString()}`)
...
export const getById = async (moduleKey, id) => {
  ...
  let url = `${module.endpoint}/${id}`
  if (EXPAND_MODULES.includes(moduleKey)) {
    // append expand param
    const sep = url.includes('?') ? '&' : '?'
    url = `${url}${sep}expand=${String(EXPAND_DEFAULT)}`
  }
